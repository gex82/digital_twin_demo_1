...
export interface CrossDockNodeData {
  shortName: string;
}
...
export function FCNode({ data, selected }: NodeProps<FcNodeData>) {
  const utilizationPct = data.utilizationPct <= 1 ? data.utilizationPct * 100 : data.utilizationPct;
  const color = utilColor(utilizationPct);
        ...
        >
          <div
            style={{
              ...
              left: 0,
              right: 0,
              height: `${Math.max(0, Math.min(100, utilizationPct)) * 0.64}%`,
              background: `${color}20`,
            }}
          />
          <span style={{ color: '#e2e8f0', fontSize: 11, fontWeight: 700, zIndex: 1 }}>{data.shortName}</span>
          <span style={{ color, fontSize: 9, zIndex: 1 }}>{utilizationPct.toFixed(0)}%</span>
        </div>
      </div>
...
export function HubNode({ data, selected }: NodeProps<HubNodeData>) {
  return (
...
export function SatelliteNode({ data, selected }: NodeProps<SatelliteNodeData>) {
  return (
...
export function CrossDockNode({ data, selected }: NodeProps<CrossDockNodeData>) {
  return (
    <div style={{ display: 'flex', flexDirection: 'column', alignItems: 'center' }}>
